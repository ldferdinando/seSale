...
function renderModal(onClose = vi.fn()) {
  ...
  render(
    <QueryClientProvider client={queryClient}>
      <ReportPlaceModal locationId={LOCATION_ID} onClose={onClose} />
    </QueryClientProvider>,
  );
  return onClose;
}

describe("ReportPlaceModal", () => {
  ...
  it("submits successfully and shows a confirmation message", async () => {
    ...
    await user.type(screen.getByLabelText("Descripción del problema"), "Este lugar cerró hace unos meses");
    await user.type(screen.getByLabelText("Tu teléfono de contacto"), "2984123456");
    await user.click(screen.getByRole("button", { name: "Enviar reporte" }));
  ...
  it("shows a rate limit message on 429", async () => {
    server.use(
      http.post(`${API_URL}/api/gastro/${LOCATION_ID}/report`, () =>
