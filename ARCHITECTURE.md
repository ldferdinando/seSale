...
GET    /api/categories/counts          Público. Query: city_id (requerido,
                                       422 si falta). Response: dict
                                       {key: count} con una entrada por
                                       categoría activa (0 incluido). Cuenta
                                       eventos status=approved, is_active=True
                                       y date >= hoy (hora Argentina) de esa
                                       ciudad. Usado por la grilla de
